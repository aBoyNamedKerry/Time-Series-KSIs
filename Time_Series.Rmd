...
Today we'll take a quick look at R's functions for looking at time series data. This document covers:

- A reminder of connecting to the TfL API from Reka's session <https://rawgit.com/maczokni/rest-api-through-firewall/gh-pages/tutorial.html>

- A reminder of using the dplyr package

- How to read in multiple years
...
We need the following packages. If you don't have them use the install.packages() function now.
...
We're going to connect to the TfL API again and extract collision data. As per last week we need to create object for our ID, Key and year we want. I'm using my appkey and id here but please use your own. 
...
We now need to collapse these lists into a single data frame. Luckily the dplyr package has a function to do this.

```{r}
 d<- rbind_all(d_list)
```

If we now `View(d)` we should see the data frame.

We have some columns we don't need for now such as the casualty list. The date is in an oddly concatenated character string. Let's remove the columns we're not interested in using the select() function in dplyr and then get the date information out in a format we can use.
...
R won't recognise our date field automatically so we need to tell it it's a date. This requires the as.Date() function. Querying ?strptime gives us a list of the characters and paramaters we need to use. We check the class before and after as a demonstration.
...
For our time series we are going to group the data by month and year. R is a little bit annoying with sorting date data so we create a new year month column.
...
You'll notice it's reverted back into a character vector so let's use the zoo package to tell R we want a Year and month column as a date. It will place a 01 day suffix day to do this.
...
It's also reverted back to a date - yay.

OK for now we're going to concentrate on looking at collisions which resulted in a killed or serious injury (KSI). We need to extract those that resulted in a fatal or serious injury. There are three ways of doing this, we can use the subset() function, or index from the data frame, data[x == "",] but we're going to use the filter() function from the dplyr and create a new variable `d_clean_KSI`.
...
Now let's create a time series object in R, ts(). We need to specify our start and end dates and also the frequency of intervals. If you put 12, the object will assume you are working in months.
...
Or if we're feeling fancy we can also build in a html widget using the dygraphs package for a bit more interactivity. Plus it fixes the odd y axis labelling.

```{r}
dygraph(ksi_ts)
```

It looks like there might be some trend in the data but it's hard to pull apart with the noise and. To check we can use a function in R called decompose() which will pull the time series into it components parts. We then use the plot function to see what this looks like.
...
There appears to be definite seasonality in the data. We can look at the seasonal components as follows.
...
We see no evidence of autocorrelation in either result. The test is non-significant so we fail to reject the null hypothesis that observation are independent and the plot does not show any auto-correlation in the lags (with the exception of one that looks like a chance observation).
...
You'll notice you'll get the same prediction for the next six months with confidence intervals. This is the same as using a control chart. The problem with using the mean of course is we have both trend and seasonality in our data which this fails to address.

OK next, we'll try a **naive forecast** taking just the last observed value.

```{r}
snaive(ksi_ts, h =6)
```

This might be slightly better but again ignores trend and seasonality.
...
Send questions to kerrycella@tfl.gov.uk or of course post them on our intranet forum page!.